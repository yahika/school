  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1000px', margin: '0 auto', fontFamily: 'Tajawal,sans-serif' }} dir="rtl">
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '28px' }}>
        ...
        <h1 style={{ color: '#5b21b6', fontWeight: 800, margin: 0, fontSize: '1.5rem' }}>💰 متابعة المصاريف</h1>
      </div>

      {/* Stats */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '16px', marginBottom: '24px' }}>
        {[
          { label: 'إجمالي الطلاب', value: fees.length, color: '#5b21b6' },
      ...
      <div style={{ background: 'white', borderRadius: '16px', padding: '24px', marginBottom: '24px', border: '1px solid #e2e8f0' }}>
        <h2 style={{ color: '#5b21b6', fontWeight: 700, marginBottom: '16px', fontSize: '1rem' }}>➕ إضافة طالب</h2>
        <form onSubmit={submit} style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '12px', alignItems: 'end' }}>
          ...
          <button type="submit" style={{ background: '#5b21b6', color: 'white', padding: '9px', borderRadius: '8px', border: 'none', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>إضافة</button>
        </form>
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
        {[{ key: 'all', label: 'الكل' }, { key: 'unpaid', label: 'لم يدفع' }, { key: 'paid', label: 'دفع' }].map(f => (
          <button key={f.key} onClick={() => setFilter(f.key)} style={{ padding: '6px 18px', borderRadius: '999px', border: '1px solid #5b21b6', background: filter === f.key ? '#5b21b6' : 'white', color: filter === f.key ? 'white' : '#5b21b6', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.85rem' }}>{f.label}</button>
        ))}
      </div>
      ...
      <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        {filtered.map((fee, i) => (
          <div key={fee.id} style={{ padding: '14px 20px', borderTop: i > 0 ? '1px solid #f1f5f9' : 'none', display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <button onClick={() => togglePaid(fee.id, fee.isPaid)} style={{ padding: '5px 12px', borderRadius: '6px', border: '1px solid #5b21b6', color: '#5b21b6', background: 'white', cursor: 'pointer', fontSize: '0.8rem', fontFamily: 'inherit' }}>
