  ...
  return (
    <div style={{ padding: '24px', maxWidth: '900px', margin: '0 auto', fontFamily: 'Tajawal,sans-serif' }} dir="rtl">
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '28px' }}>
        ...
        <h1 style={{ color: '#5b21b6', fontWeight: 800, margin: 0, fontSize: '1.5rem' }}>📅 جدول الامتحانات</h1>
      </div>
      ...
      <div style={{ background: 'white', borderRadius: '16px', padding: '24px', marginBottom: '24px', border: editId !== null ? '2px solid #5b21b6' : '1px solid #e2e8f0' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <h2 style={{ color: '#5b21b6', fontWeight: 700, margin: 0, fontSize: '1rem' }}>{editId !== null ? '✏️ تعديل الامتحان' : '➕ إضافة امتحان'}</h2>
        ...
        <form onSubmit={submit} style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '12px' }}>
          ...
          <div style={{ gridColumn: 'span 3' }}>
            <button type="submit" style={{ background: '#5b21b6', color: 'white', padding: '10px 28px', borderRadius: '8px', border: 'none', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
        ...
        {items.map((item, i) => (
          <div key={item.id} style={{ padding: '14px 20px', borderTop: i > 0 ? '1px solid #f1f5f9' : 'none', display: 'flex', alignItems: 'center', gap: '16px', background: editId === item.id ? '#f0fdf4' : 'white' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => startEdit(item)} style={{ padding: '5px 12px', borderRadius: '6px', border: '1px solid #5b21b6', color: '#5b21b6', background: 'white', cursor: 'pointer', fontSize: '0.8rem', fontWeight: 600 }}>✏️ تعديل</button>
      ...
      {toast && <div style={{ position: 'fixed', bottom: '24px', left: '50%', transform: 'translateX(-50%)', background: '#5b21b6', color: 'white', padding: '12px 24px', borderRadius: '999px', fontWeight: 600, zIndex: 999 }}>{toast}</div>}
    </div>
  )
}
