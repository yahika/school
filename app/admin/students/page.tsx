  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1100px', margin: '0 auto', fontFamily: 'Tajawal,sans-serif' }} dir="rtl">
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '28px', flexWrap: 'wrap' }}>
        ...
        <h1 style={{ color: '#5b21b6', fontWeight: 800, margin: 0, fontSize: '1.5rem' }}>👨‍🎓 دليل الطلاب</h1>
        <span style={{ background: '#f0fdf4', color: '#5b21b6', fontWeight: 700, fontSize: '0.85rem', padding: '4px 14px', borderRadius: '999px', border: '1px solid #bbf7d0' }}>{students.length} طالب</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {filtered.map(s => (
                <div key={s.seatNumber} onClick={() => selectStudent(s)} style={{
                  ...
                  border: selected?.seatNumber === s.seatNumber ? '2px solid #5b21b6' : '1px solid #e2e8f0',
                ...
                >
                  <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'linear-gradient(135deg,#5b21b6,#6d28d9)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 800, fontSize: '0.8rem', flexShrink: 0 }}>
                    {s.seatNumber}
                  </div>
                  <div style={{ flex: 1 }}>
                  ...
                  {s.latestPct > 0 && (
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '1.2rem', fontWeight: 900, color: s.latestPct >= 85 ? '#5b21b6' : s.latestPct >= 60 ? '#d97706' : '#dc2626' }}>{s.latestPct}%</div>
        ...
        {selected && (
          <div style={{ position: 'sticky', top: '24px' }}>
            <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
              <div style={{ background: 'linear-gradient(135deg,#5b21b6,#6d28d9)', padding: '24px', color: 'white', position: 'relative' }}>
                    ...
                    {results.map((r, i) => (
                      <div key={i} style={{ background: '#f8fafc', borderRadius: '10px', padding: '14px', border: '1px solid #e2e8f0' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                          ...
                          <span style={{ fontWeight: 900, fontSize: '1.1rem', color: r.percentage >= 85 ? '#5b21b6' : r.percentage >= 60 ? '#d97706' : '#dc2626' }}>{r.percentage}%</span>
                        </div>
                        <div style={{ height: '6px', background: '#e2e8f0', borderRadius: '999px', overflow: 'hidden' }}>
                          <div style={{ height: '100%', width: `${r.percentage}%`, background: r.percentage >= 85 ? '#5b21b6' : r.percentage >= 60 ? '#d97706' : '#dc2626', borderRadius: '999px' }} />
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '6px' }}>
                          ...
                          <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                            ...
                            <button onClick={(e) => { e.stopPropagation(); downloadPDF(r.id, selected!.seatNumber) }} style={{ fontSize: '0.7rem', padding: '2px 8px', borderRadius: '6px', border: '1px solid #5b21b6', color: '#5b21b6', background: 'white', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600 }}>
                              📄 PDF
                            </button>
                          </div>
                        </div>
